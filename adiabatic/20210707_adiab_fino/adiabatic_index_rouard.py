...
sample_num = 1
t_ref, E_ref = read_1file('./ref' + str(sample_num) + '.txt')
E_ref = - E_ref
t_sam, E_sam = read_1file('./sam' + str(sample_num) + '.txt')
...
f_sam[0] = 1
# plot(irfft(E_sam_w / E_ref_w * wiener_filter(E_ref_w, beta=1e-3)))
# show()
# quit()
# print(f_ref * 1e-12)
# quit()
D_adiab = 135e-6  # 100 um
n_1 = 1.45 - 1j * 0.03  # blau
thick_1 = 270e-6  # - D_adiab/2  # 1000 um
n_2 = 1.55 - 1j * 0.06  # groc
thick_2 = 290e-6  # - D_adiab/2  # 1000 um
...
for D_adiab in [D_adiab]:   # , 1e-4, 5e-4]:  # , 1e-4, 1.5e-4]:  # , 1e-3]:  # 1000 és suficient per a simular adiabàtic
...
plot(t_sam, irfft(E_sam_w / E_ref_w * wiener_filter(E_ref_w, beta=1e-5)), '--', label='sam', lw=1)
# plot(irfft(E_sam_w / E_ref_w * wiener_filter(E_ref_w, beta=1e-5)), '--', label='sam', lw=1)
# xlim([1, 4])
# ylim([-0.005, 0.001])
