...
def D_i_j(n_i, n_j):
    t_12 = t_i_j(n_i, n_j)
    inv_t_12 = 1 / t_12
    R_ij = array(((1, r_i_j(n_i, n_j)), (r_i_j(n_i, n_j), 1)))
    aux = list()
    for i in range(inv_t_12.size):
        aux.append(complex(inv_t_12[i]) * R_ij[i])
...
t_ref, E_ref = read_1file('./adiabatico_3capas/20210628_adiabatic/ref1.txt')
...
D_adiab = 1e-4  # 100 um
n_1 = 1.55 * ones(f_ref.size) - 1j * 0.3 * f_ref * 1e-12
thick_1 = 1.5e-4  # - D_adiab/2  # 1000 um
n_2 = 1.45 * ones(f_ref.size) - 1j * 0.3 * f_ref * 1e-12
thick_2 = 1.5e-4  # - D_adiab/2  # 1000 um
