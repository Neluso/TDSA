...
for N_grid in [2, 1000]:
    ...
    for freq in f_ref:
        ...
        n_in = dot(P_i(n_1, thick_1, freq), D_i_j(n_air, n_1))
        m_i = m_in
        for i in range(N_grid):
            ...
            m_i = dot(m_j, m_i)
        m_in = dot(P_i(n_1, thick_1 - D_adiab / 2, freq), D_i_j(n_air, n_1))
        n_in = dot(P_i(n_1, thick_1, freq), D_i_j(n_air, n_1))
        m_i = m_in
        for i in range(N_grid - 1):
            n_i = n_adiab[i]  # f_n_1(n_1, n_2, D_adiab, d * (i + 0.5))  # n_i
            n_i_1 = n_adiab[i + 1]  # f_n_1(n_1, n_2, D_adiab, d * (i + 1.5))  # n_i+1
            m_j = dot(P_i(n_i, d, freq), D_i_j(n_i, n_i_1))
            m_i = dot(m_j, m_i)
        n_i = n_adiab[N_grid - 1]
        n_i_1 = n_2  # n_i+1
        ...
        n_i = n_2
        n_i_1 = n_air  # n_i+1
        phase_mat_i = P_i(n_i, thick_2 - D_adiab / 2, freq)
        phase_nat_i = P_i(n_i, thick_2, freq)
        fres_mat_i = D_i_j(n_i, n_i_1)
        m_out = dot(phase_mat_i, fres_mat_i)
        n_out = dot(phase_nat_i, fres_mat_i)
        m_i = dot(m_out, m_i)
        n_mat_i = dot(n_out, n_in)
        M_i.append(m_i)
        N_i.append(n_mat_i)
